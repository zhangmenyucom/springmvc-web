			...
			while (true) {
				...
				if (resp.getError() == 0) {
					...
					if (cureentCodeNum.equals(lastesCodeNum)) {
						...
						lastNum = cureentNum;
						singapore30OrderPost.postOrder("dxdsh",initMutiply, lastNum % 2 == 0 ? "双,双单" : "单,单双");
						System.out.println("加注成功，加注倍数"+initMutiply+"下注方案："+(lastNum % 2 == 0 ? "双,双单" : "单,单双")+",操作时间:"+new SimpleDateFormat("YYYY-MM-dd HH:mm:ss").format(new Date()));
					}
				} else {
