...
public class SigaporeAppQianShiWei extends Thread {
	public static void main(String[] args) {
		startThread();
	}

	public void run() {
		System.out.println("***************************前二星压【十】位启动成功*********************************");
		startThread();
	}
				...
				if (resp.getError() == 0) {
					...
					if (cureentCodeNum.equals(lastesCodeNum)) {
						...
						if (evenTimes >= Constants.inverseNum) {
							/** 持续多次双后开始投单 **/
							if (!isReverse) {
								singapore30OrderPost.postOrder("dxdsq", initMutiply, "单,单双");
								strategyName = "连续" + evenTimes + "次双后压单";
							} else {
								singapore30OrderPost.postOrder("dxdsq", initMutiply, "双,单双");
							...
							if (!isReverse) {
								...
								strategyName = "按上次开奖下注：" + (lastNum % 2 == 0 ? "双,单双" : "单,双单");
							} else {
								singapore30OrderPost.postOrder("dxdsq", initMutiply, lastNum % 2 == 0 ? "单,单双" : "双,单双");
