...
public class SigaporeAppQianGeWei extends Thread {
	...
	public void run() {
		System.out.println("***************************前二星压【个】位启动成功*********************************");
		startThread();
			...
			while (true) {
				...
				if (resp.getError() == 0) {
					...
					if (cureentCodeNum.equals(lastesCodeNum)) {
						...
						System.out.println("方案：大小单双前二星压个位---最新一期号码：" + gameOpenCode.getIssue() + "开奖码：" + gameOpenCode.getCode() + ",尾数为：------" + cureentNum + "--------" + (cureentNum % 2 == 0 ? "双" : "单"));
